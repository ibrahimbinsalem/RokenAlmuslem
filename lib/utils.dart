  ...
  try {
    ...
    for (var ayahI = 0; ayahI < ayahsData.length; ayahI++) {
      ...
      var surahTranslates = QuranTranslate(
        translates: {TranslateID.indonesia: ayahsTranslateData[ayahI]['text']},
      );
...
Future<AyahData> gettingAyah(int number) async {
  try {
    ...
    finalAyah.translate = QuranTranslate(
      translates: {TranslateID.indonesia: dataTranslate['text']},
    );
...
class DioHelper {
  static late Dio dio;
  static init() {
    dio = Dio(BaseOptions(baseUrl: 'http://api.alquran.cloud/v1/'));
  }
